...
- `6fc6b80a7c0d4eb51c92a5b3857fd3cd1f9b2d2e`
  - Message: `Add FO2 gas config deco ladder UI and alert updates`
  - 주요 내용:
    - FO2 gas config 확정
    - DECO ladder UI 연동
    - CEIL >18m 표시
    - ASCEND/DESCEND 화살표 UI
    - LOW BATTERY 10분 팝업
    - battery low beep
    - fast ascent triple beep
    - Wokwi buzzer test command
    - monitor_port 추가

- `1088450ffd69c4a94685768d9d1f4ae499fcc314`
  - Message: `Add FO2 based gas config and deco ladder handling`
  - 주요 내용:
    - Bühlmann FN2 하드코딩 제거
    - FO2 기반 FN2 계산
    - MOD / ppO2 helper 추가
    - DECO ladder helper 추가
    - raw ceiling >18m 처리 구조 추가
...
`v1.3.5-dev` 태그 기준으로 다음 기능이 코드에 반영되어 있다.

### DECO.STOP violation / advisory

- DECO.STOP 미완료 상태에서 출수하면 `activeDecoViolation_` 설정
- 48시간 `postViolationAdvisory_` 시작
- advisory 종료 시각은 `postViolationAdvisoryEndEpochSec_`에 저장
- 출수 후 PostDive 화면에서 `MISSED DECO` alert 표시
- alert 표시 시간은 `DECO_VIOLATION_ALERT_DISPLAY_MS = 30000ms`
- alert 이후 SURFACE 화면으로 전환
- SURFACE 화면에서는 기존 LAST/MAX/TMP/SURFACE/N-FLY 정보를 유지
- 마지막 줄에서 `N-FLY`와 `DECO.VIOL`을 2초 간격으로 교대 표시
- 재입수 시 hard lockout 없이 tissue state 기반으로 DECO.STOP 재계산
- 재입수 이벤트 `EVENT_DECO_REENTRY` 기록
- 재입수 후 감압 clear 시 `EVENT_DECO_CLEARED_AFTER_REENTRY` 기록

### DECO.STOP margin

DECO.STOP은 대칭 ±0.6m가 아니라 아래 비대칭 margin을 사용한다.
...
예: DECO.STOP 9m
```text
8.4m보다 얕음       → DESCEND, timer pause
8.4m ~ 9.6m        → HOLD, timer run
9.6m ~ 10.8m       → ASCEND, timer run
10.8m보다 깊음     → ASCEND, timer pause
```

Surface / PostDive flow after missed DECO
``` text
수심 < 0.5m
↓ 60초 유지
최종 출수 확정
...
SURFACE 화면 전환
↓
SURFACE 화면에서 DECO.VIOL + 남은 advisory 시간 표시
```
...
## 27.1 2026-05-07 현재 코드 반영 상태

현재 `dev/v1.3` 브랜치에는 다음 코드 변경이 적용되어 있다.

완료된 항목:

```text
include/config.h에 FO2/ppO2 설정 추가
FO2 기본값 21%, ppO2 max 1.4 bar 적용
FO2 허용 범위 21~40% compile-time 검사 추가
Bühlmann 계산에서 FN2 하드코딩 제거
getGasFO2(), getGasFN2() helper 추가
calculateMODMeters(), calculatePpO2Bar() helper 추가
DECO.STOP ladder 18/15/12/9/6/3m 추가
raw ceiling을 ladder stop으로 매핑
ceiling >18m일 경우 CEIL >18m 경고 처리
uiDrawDiveDeco()에서 CEIL >18m 화면 표시
ASCEND/DESCEND/HOLD 행동 지시 적용
Unicode 화살표 대신 u8g2.drawTriangle() 사용
LOW BATTERY 팝업 주기를 10분으로 변경
LOW BATTERY 비프 1회 호출 추가
빠른 상승 위험 시 3회 비프 호출 추가
Wokwi beep test 명령 추가
diagram.json buzzer smooth mode 및 배선 정리
platformio.ini에 monitor_port 추가
```

아직 남은 항목:

```text
로컬 pio run -e wokwi 빌드 검증
Wokwi 환경에서 실제 buzzer 음성 출력 확인
FO2 >21%일 때 MOD 상시 표시 UI
MOD 초과 경고 UI
missed deco 상태 머신 완성
48시간 advisory UI와 로그 이벤트 완성
re-entry 시 tissue state 유지 및 DECO.STOP 재계산 검증
DECO.STOP completed / next stop 자동 전환 고도화
gas 설정을 scenario JSON 또는 향후 BLE 앱 설정과 연결
```
